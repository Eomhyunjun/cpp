...
bool Form::isExecutable(Bureaucrat const& _Bureaucrat) const
{
	if (issigned == false)
        throw UnapprovedException();
    if (_Bureaucrat.getGrade() > executeGrade)
        throw GradeTooLowExcute();
    return true;
}
...
{
	return ("~~~~UnapprovedException~~~~~");
}

const char* Form::GradeTooLowExcute::what() const throw()
{
	return ("~~~~GradeTooLowExcute~~~~~");
}
